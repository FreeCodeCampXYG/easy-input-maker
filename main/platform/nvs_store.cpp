...
bool NvsConfigStore::load_song(
    std::array<std::uint8_t, ai_keyboard::kMusicSequencePayloadLen>* payload,
    esp_err_t* out_err) const {
  if (payload == nullptr) { set_error(out_err, ESP_ERR_INVALID_ARG); return false; }
  nvs_handle_t handle = 0;
  esp_err_t err = nvs_open(kPrefsNamespace, NVS_READONLY, &handle);
  std::size_t length = payload->size();
  if (err == ESP_OK) err = nvs_get_blob(handle, kPrefsSongKey, payload->data(), &length);
  if (handle != 0) nvs_close(handle);
  if (err != ESP_OK || length != payload->size()) {
    set_error(out_err, err == ESP_OK ? ESP_ERR_INVALID_SIZE : err);
    return false;
