...
所有帧固定 16 字节、小端，首字节为版本 `1`。Control 帧含命令、能力、request id、lease 和显式 Exclusive 标志。ACK 返回结果、实际授予能力、lease 与 session generation。未知版本、长度或枚举值必须拒绝。
...
- `Exclusive` 必须有显式请求，且主任务确认没有 held keyboard、待投递 bridge hotkey 或 text-selection chord 后才能接受。拒绝不会清空 PC 状态；断线或过期回退到原 PC 路由。
