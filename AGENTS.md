...
- 在 `course/host-action-v1-starter` 中，Host Action 缺失是课程起点合同，不是待修回归；只有学员明确进入对应课程节点时才逐步实现，不能直接把 `main` 的答案提交整笔搬回。
...
0. 按 `docs/development/README.md` 的 L0→L1→L2 读取：先确认功能 ID，再读对应功能卡；修改共享文件必须反查全部消费者并扩大回归。数字槽位不是稳定功能身份，不因文档分层假设代码已解耦。
